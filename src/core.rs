...
fn assume(env: &mut Env, name: &str, sig: &str) {
    let (vars, ty) = Parser::ty(sig).unwrap();
    let ty = env.replace_ty_constants_with_vars(vars, ty);
...
pub fn make_env() -> Env {
    ...
    assume(&mut env, "cons_curry", "forall[a] a -> list[a] -> list[a]");
    assume(&mut env, "map", "forall[a b] (a -> b, list[a]) -> list[b]");
    assume(
        &mut env,
        "map_curry",
